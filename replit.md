...
- **Split Session Management (November 2025)**: Implemented split lifecycle tracking with "open" and "closed" states stored in localStorage (client/src/lib/split-status.ts). Features include: "New Split" button on split-bill page with confirmation dialog to prevent accidental data loss when starting fresh splits; "Close Split" / "Reopen Split" toggle buttons on both results and view-split pages with visual feedback (destructive variant for close, default for reopen); "Active Splits" section on home page displaying all open splits with green indicator dots that link directly to split detail pages; defensive parsing with schema validation in split-status helpers to prevent crashes from malformed localStorage data; automatic cleanup of invalid entries; and cross-tab synchronization via storage events to keep active splits in sync across browser tabs
